...
class WorksAdapter(val context: Context, private val onButtonClicked: ((AssignedWork) -> Unit)? = null, private val buttonText: String
    ...
    override fun onBindViewHolder(holder: WorksViewHolder, position: Int) {
        ...
        when (work.workPriority) {
            "1" -> holder.binding.cvOval.setBackgroundResource(R.drawable.red_oval)
            "2" -> holder.binding.cvOval.setBackgroundResource(R.drawable.yellow_oval)
            "3" -> holder.binding.cvOval.setBackgroundResource(R.drawable.green_oval)
        }
        holder.binding.cvOval.setOnClickListener {
            when (work.workPriority) {
                "1" -> Toast.makeText(context, "Priority : High", Toast.LENGTH_SHORT).show()
                "2" -> Toast.makeText(context, "Priority : Medium", Toast.LENGTH_SHORT).show()
                "3" -> Toast.makeText(context, "Priority : Low", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun isAnyItemExpanded(position: Int) {
        val temp = worksList.indexOfFirst { it.isExpandable }
